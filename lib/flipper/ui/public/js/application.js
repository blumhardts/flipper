document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("click", function (e) {
    ...
    if (e.target.classList.contains("js-add-expression-group")) {
      ...
      // Initialize remove button tooltip
      var removeExpressionButtons = expressions.querySelectorAll(".js-remove-expression");
      var removeExpressionButton = removeExpressionButtons[removeExpressionButtons.length - 1];
      new bootstrap.Tooltip(removeExpressionButton);
    ...
    if (e.target.classList.contains("js-remove-expression")) {
      ...
      if (expressionGroup) {
        var otherExpression = expressionGroup.querySelector(".expression");

        if (
          !otherExpression ||
          (
            otherExpression &&
            otherExpression.closest(".expression-group") != expressionGroup
          )
        ) {
      ...
      if (!form.querySelector(".expression")) {
